...
class KNNClassifier:
    ...
    def fit(self, Devs, tags):
        self.Devs = self.preprocess(Devs)
        self.tags = tags
        print("selfdevsTest",self.Devs,"selfdevsTest")
    ...
    def euclidianD(self, firstRow, secondRow):
        ...
        while i < len(firstRow):
            print("firstRow",firstRow[i],"secondRow",secondRow[i])
    ...
    def preprocess(self, newIncomers):
       for index, row in newIncomers.iterrows():
           newIncomers.at[index, 'role'] = np.sum(self.embeddings_dict[row.role.lower()])
           newIncomers.at[index, 'hobby'] = np.sum(self.embeddings_dict[row.hobby.lower()])
           newIncomers.at[index, 'scholarity'] = np.sum(self.embeddings_dict[row.scholarity.lower()])
...
if __name__ == "__main__":
    ...
    print(Devs,"testeDevs2")
    AllnewIncomers = pd.read_json("allnewIncomers.json")
    y_true = AllnewIncomers.productivity_class
    AllnewIncomers = AllnewIncomers.drop(columns=["id", "name","productivity_class"])
