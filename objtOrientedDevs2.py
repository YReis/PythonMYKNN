...
class KNNClassifier:
    ...
    def euclidianD(self, firstRow, secondRow):
        ...
        while i < len(firstRow):
            print("first",firstRow[i],"second",secondRow[i])
    ...
    def _predict(self, newIncomer):
        newIncomer = self.preprocess(pd.DataFrame([newIncomer]))  # Convert the Series to DataFrame
        arrNewincomerDists = [(index, self.euclidianD(newIncomer.values[0], row)) for index, row in self.Devs.iterrows()]
    ...
    def preprocess(self, newIncomers):
       for index, row in newIncomers.iterrows():
           newIncomers.at[index, 'role'] = self.embeddings_dict[row.role.lower()]
           newIncomers.at[index, 'hobby'] = self.embeddings_dict[row.hobby.lower()]
           newIncomers.at[index, 'scholarity'] = self.embeddings_dict[row.scholarity.lower()]
    ...
    def predict(self, newIncomers):
        ...
        newIncomersClassified = [self._predict(newIncomer) for _, newIncomer in newIncomers.iterrows()]
